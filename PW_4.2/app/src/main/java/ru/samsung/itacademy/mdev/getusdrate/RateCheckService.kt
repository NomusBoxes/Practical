...
class RateCheckService : Service() {
    ...
    val rateCheckRunnable: Runnable = Runnable{
        ...
        if (rateCheckAttempt > RATE_CHECK_ATTEMPTS_MAX) {
            Toast.makeText(applicationContext, "Max attempts count reached, stopping service", Toast.LENGTH_SHORT).show()
            stopSelf()
            return@Runnable
        }
        requestAndCheckRate()
    ...
    companion object {
        ...
        const val RATE_CHECK_INTERVAL = 1000L//5000L
        const val RATE_CHECK_ATTEMPTS_MAX = 11//100
